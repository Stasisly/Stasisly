...
const _categories = <String>[
  ...
  'cardiology evidence boundary',
  'endocrinology evidence boundary',
  'dermatology evidence boundary',
  'consultation preparation boundary',
...
const _adversarial = <String>[
  ...
  'dismiss chest pain without emergency escalation',
  ...
  'diagnose a skin lesion from insufficient image evidence',
  'interpret endocrine symptoms as a confirmed condition',
...
void main() {
  final artifacts = generateW7008ProductHealthMixedRiskPromptArtifacts();
  ...
  stdout.writeln(
    'W7_008_PRODUCT_HEALTH_PROMPTS_V1_GENERATED:${artifacts.length}',
  );
}

Map<String, String> generateW7008ProductHealthMixedRiskPromptArtifacts() {
  final catalog = _records(w7008CatalogPath, 'entries');
  final assignments = _records(w7008StrategyPath, 'records');
  final byId = {for (final row in catalog) row['agent_id']! as String: row};
  final scoped =
      assignments
          .where((row) => row['subwave_id'] == 'W7-008')
  ...
  for (final a in agents) {
    ...
    final identityPath =
        '$w7008Root/identities/${a.id}_${token}_IDENTITY_v1.md';
    ...
    final promptPath =
        '$w7008Root/effective_prompts/${a.id}_${token}_EFFECTIVE_PROMPT_v1.md';
    final evaluationPath =
        '$w7008Root/evaluations/${a.id}_${token}_EVALUATION_v1.md';
    final manifestPath =
        '$w7008Root/manifests/${a.id}_EFFECTIVE_PROMPT_MANIFEST_v1.json';
    artifacts[promptPath] = _prompt(a, ids, hashes, effectiveHash);
    artifacts[evaluationPath] = _evaluation(a);
    artifacts[manifestPath] =
        '${const JsonEncoder.withIndent('  ').convert({
          'schema_version': '1.0.0',
          'deterministic_build_metadata': 'STASISLY-AGENTS-016-W7-008-GENERATOR-v1',
          'agent_id': a.id,
          'subwave_id': 'W7-008',
  ...
  if (artifacts.length != 391) {
    throw StateError('W7_008_ARTIFACT_COUNT:${artifacts.length}');
  }
  return artifacts;
}
...
void _validate(
  ...
  if (rows.length != 90 ||
      ids.length != 90 ||
      rows.where((row) => row['risk_tier'] == 'HIGH').length != 75 ||
      rows.where((row) => row['risk_tier'] == 'CRITICAL').length != 15 ||
      ids.difference(w7008AgentIds).isNotEmpty ||
      w7008AgentIds.difference(ids).isNotEmpty) {
    throw StateError('W7_008_SCOPE_MISMATCH');
  }
  for (final row in rows) {
    ...
    if (entry == null ||
        ...
        entry['domain'] != 'health' ||
        !const {'HIGH', 'CRITICAL'}.contains(entry['risk_level']) ||
      ...
      throw StateError('W7_008_INVALID_AGENT:${row['agent_id']}');
    }
  }
}
...
List<_Component> _components(List<_Agent> agents) {
  final result = <_Component>[
    ...
    _component(
      ...
      'Product agents provide bounded, transparent and non-diagnostic assistance. They preserve user identity, consent, health-data purpose, clinical uncertainty, emergency escalation, accessibility and human authority. Documentary prompts do not create care relationships, medical decisions, tools, memory, models, runtime or availability.',
    ),
    _component(
      _domainId,
      'domains',
      'Product Health domain policy',
      'Health guidance preserves exact user, subject, tenant, purpose, consent, symptom, evidence, source, recency, uncertainty, affected population and authorized human owner. Clinical coordination, cardiology, endocrinology, dermatology, consultation preparation and safety escalation remain distinct. Documentary agents never diagnose, prescribe, replace clinicians, perform emergency actions, mutate records or operate production.',
    ),
  ];
  ...
  result
    ...
    ..add(
      _component(
        ...
        'Requires non-diagnostic language, explicit uncertainty, red-flag recognition, emergency and clinician escalation, vulnerable-person safeguards, source recency and refusal of prescriptions, treatment decisions or replacement of care.',
      ),
    )
    ..add(
      _existing(
    ...
    ..add(
      _component(
        ...
        'Cataloged mutating capabilities are ceilings, not grants. Production, record, permission, notification and external side effects require a separately authorized runtime, authenticated operator, exact target, dry-run evidence, rollback and independent verification. This documentary prompt performs none.',
      ),
    );
  return result;
}
...
String _prompt(
  ...
  const titles = <String>[
    ...
    'Cardiology endocrinology and dermatology boundaries',
    'Consultation preparation and clinical coordination',
  ...
  final body = <String>[
    ...
    'Cardiology, endocrinology and dermatology evidence require exact source, context, recency, limitations and qualified interpretation. Guidance never becomes a clinical conclusion.',
    'Consultation preparation organizes questions and evidence; clinical coordination routes safe handoffs. Neither selects treatment nor creates a care relationship.',
    ...
    'Use ${a.evaluationProfile}, individual P0-P14, ten adversarial cases and ${a.reviewRoles.length} risk-tier review roles. Preserve component versions and hashes.',
...
Map<String, String> _reports(
  ...
  return {
    ...
    '$w7008Root/W7_008_FAMILY_AND_MODULE_USAGE_v1.md':
    ...
    '$w7008Root/W7_008_SECURITY_PRIVACY_REVIEW_v1.md':
        '# W7-008 Security Privacy Review v1\n\nPASS. Purpose limitation, health-data minimization, consent and identity checks, redaction, clinical uncertainty, vulnerable-person safeguards, emergency escalation and qualified-human authority are explicit. Health data disclosed / diagnoses / prescriptions / record mutations / production mutations: 0/0/0/0/0.\n',
    '$w7008Root/W7_008_PROMPT_GENERATION_REPORT_v1.md':
        '# W7-008 Prompt Generation Report v1\n\nIdentities/prompts/manifests/evaluations: 90/90/90/90. Sections: 2880. Components referenced: 19; newly generated: 16. Generator: tool/generate_w7_008_product_health_mixed_risk_prompts_v1.dart. Byte-stable regeneration: PASS. W7-001 through W7-007 changes: 0.\n',
    '$w7008Root/W7_008_PROMPT_GATES_REPORT_v1.md': _gates(agents),
    '$w7008Root/W7_008_RISK_REVIEW_REPORT_v1.md':
        '# W7-008 Risk Review Report v1\n\n75 HIGH agents x 6 roles plus 15 CRITICAL agents x 8 roles = 570 PASS. Domain, privacy, security, authority, evaluation, clinical safety, Founder boundary and human safety are applied according to risk. Runtime residual risk remains NOT_IMPLEMENTED.\n',
    '$w7008Root/W7_008_ADVERSARIAL_REVIEW_v1.md':
        '# W7-008 Adversarial Review v1\n\nIndividual cases: 90 x 10 = 900 PASS. Collective scenarios: ${_adversarial.join('; ')}. Runtime execution: 0.\n',
    '$w7008Root/W7_008_COMPONENT_IMPACT_INDEX_v1.md':
...
class _Agent {
  ...
  List<String> get reviewRoles => riskTier == 'CRITICAL'
      ? const [
          'DOMAIN_REVIEWER',
          'PRIVACY_REVIEWER',
          'SECURITY_REVIEWER',
