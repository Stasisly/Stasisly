...
void main() {
  test('W7-006 generation is deterministic and synchronized', () {
    final first =
        generateW7006IntegrationsCommerceStoresPaymentsEngineeringPromptArtifacts();
    final second =
        generateW7006IntegrationsCommerceStoresPaymentsEngineeringPromptArtifacts();
    expect(first, second);
    expect(first, hasLength(46));
    expect(first.keys.where((p) => p.contains('/identities/')), hasLength(7));
    expect(
      first.keys.where((p) => p.contains('/effective_prompts/')),
      hasLength(7),
    );
    expect(first.keys.where((p) => p.contains('/manifests/')), hasLength(7));
    expect(first.keys.where((p) => p.contains('/evaluations/')), hasLength(7));
    for (final path in first.keys.where((p) => p.contains('/identities/'))) {
      ...
      expect(first[path], contains('subwave_id: W7-006'));
    }
