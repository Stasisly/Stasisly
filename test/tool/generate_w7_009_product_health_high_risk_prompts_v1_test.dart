...
void main() {
  test('W7-009 generation is deterministic and synchronized', () {
    final first = generateW7009ProductHealthHighRiskPromptArtifacts();
    final second = generateW7009ProductHealthHighRiskPromptArtifacts();
    expect(first, second);
    ...
    for (final path in first.keys.where((p) => p.contains('/identities/'))) {
      ...
      expect(first[path], contains('subwave_id: W7-009'));
    }
