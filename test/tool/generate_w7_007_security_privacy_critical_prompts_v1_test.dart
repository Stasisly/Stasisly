...
void main() {
  test('W7-007 generation is deterministic and synchronized', () {
    final first = generateW7007SecurityPrivacyCriticalPromptArtifacts();
    final second = generateW7007SecurityPrivacyCriticalPromptArtifacts();
    expect(first, second);
    expect(first, hasLength(436));
    expect(first.keys.where((p) => p.contains('/identities/')), hasLength(100));
    expect(
      first.keys.where((p) => p.contains('/effective_prompts/')),
      hasLength(100),
    );
    expect(first.keys.where((p) => p.contains('/manifests/')), hasLength(100));
    expect(
      first.keys.where((p) => p.contains('/evaluations/')),
      hasLength(100),
    );
    for (final path in first.keys.where((p) => p.contains('/identities/'))) {
      ...
      expect(first[path], contains('subwave_id: W7-007'));
    }
    ...
    for (final path in first.keys.where(
      (p) => p.contains('/composable/') && !p.contains('/overlays/'),
    )) {
      expect(first[path], contains('artifact_type:'));
      expect(first[path], contains('risk_compatibility: CRITICAL'));
