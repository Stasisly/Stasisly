...
void main() {
  test('W7-005 generation is deterministic and synchronized', () {
    final first =
        generateW7005DevopsSreObservabilityIncidentCommandPromptArtifacts();
    final second =
        generateW7005DevopsSreObservabilityIncidentCommandPromptArtifacts();
    expect(first, second);
    expect(first, hasLength(59));
    expect(first.keys.where((p) => p.contains('/identities/')), hasLength(10));
    expect(
      first.keys.where((p) => p.contains('/effective_prompts/')),
      hasLength(10),
    );
    expect(first.keys.where((p) => p.contains('/manifests/')), hasLength(10));
    expect(first.keys.where((p) => p.contains('/evaluations/')), hasLength(10));
    for (final path in first.keys.where((p) => p.contains('/identities/'))) {
      ...
      expect(first[path], contains('subwave_id: W7-005'));
    }
