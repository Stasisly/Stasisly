...
void main() {
  ...
  setUpAll(() {
    catalog = _records(w7008CatalogPath, 'entries');
    assignments = _records(w7008StrategyPath, 'records');
    identities = Directory(
      '$w7008Root/identities',
    ).listSync().whereType<File>().toList();
    prompts = Directory(
      '$w7008Root/effective_prompts',
    ).listSync().whereType<File>().toList();
    manifests = Directory(
      '$w7008Root/manifests',
    ).listSync().whereType<File>().toList();
    evaluations = Directory(
      '$w7008Root/evaluations',
    ).listSync().whereType<File>().toList();
  });

  test('scope resolves exactly 90 mixed-risk Product health agents', () {
    final scoped = assignments
        .where((r) => r['subwave_id'] == 'W7-008')
        .toList();
    expect(scoped, hasLength(90));
    expect(scoped.map((r) => r['agent_id']).toSet(), w7008AgentIds);
    expect(scoped.where((r) => r['risk_tier'] == 'HIGH'), hasLength(75));
    expect(scoped.where((r) => r['risk_tier'] == 'CRITICAL'), hasLength(15));
  ...
  test('CSV JSON and Markdown preserve the exact W7-008 plan', () {
    ...
    final row = plan.singleWhere((r) => r['subwave_id'] == 'W7-008');
    expect(row['agent_count'], 90);
    expect(row['surface'], 'PRODUCT');
    expect(row['domain'], 'health');
    expect(row['HIGH_count'], 75);
    expect(row['CRITICAL_count'], 15);
    ...
    expect(RegExp('^"W7-008",', multiLine: true).allMatches(csv), hasLength(1));
    expect(
      RegExp(r'^\| W7-008 \|', multiLine: true).allMatches(markdown),
      hasLength(1),
    );
  });
  ...
  test('evaluations contain 900 adversarial cases and 570 risk reviews', () {
  ...
  test('1350 P0-P14 pass and P15-P17 remain unexecuted', () {
    final gates = File(
      '$w7008Root/W7_008_PROMPT_GATES_REPORT_v1.md',
    ).readAsStringSync();
    expect(
      RegExp(
  ...
  test('reports and ADR-RF117 through RF123 are present', () {
    expect(Directory(w7008Root).listSync().whereType<File>(), hasLength(15));
    for (var i = 117; i <= 123; i++) {
  ...
  test('closed and future subwaves remain untouched', () async {
    final result = await Process.run('git', [
      'diff',
      '--name-only',
      '3cc69fe97307eae1c1fa763f88474949681ca27c',
      '--',
