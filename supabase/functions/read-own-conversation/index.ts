...
async function invoke(
  ...
  const response = await fetcher(
    new URL("/rest/v1/rpc/read_own_conversation_core", baseUrl),
    {
      method: "POST",
      headers: {
        apikey: key,
  ...
  return async (request) => {
    ...
    try {
      ...
      const authorization = await prepareBackendAuthorization({
        ...
        fetcher,
        config,
        operation: BACKEND_OPERATIONS.readOwnConversation,
      ...
      const conversation = sanitizeConversation(rows, conversationId);
      finalizeBackendAuthorization(
        authorization,
        BACKEND_OPERATIONS.readOwnConversation,
        "owned",
      );
